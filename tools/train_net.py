...
def change_yolox_label_to_glip_label(jsonfile='instances_train_0193.json'):
    f = open(jsonfile, 'r')
    cocodt_dataset_ann = json.load(f)['annotations']
    f = open("/data2/wyj/GLIP/DATASET/coco/annotations/instances_train2017_glipGT.json", 'r')
    cocodt_dataset = json.load(f)
    cocodt_dataset['annotations'] = cocodt_dataset_ann
    f = open("/data2/wyj/GLIP/DATASET/coco/annotations/instances_train2017.json", 'w')
    json.dump(cocodt_dataset, f)
def main():
    # import time
    # time.sleep(3600*10)
    ...
    change_yolox_label_to_glip_label(args.train_label)
    if args.distributed:
        import datetime
