...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.mlp_a=True
        self.mlp_b=True
        self.mlp_c=False# caution: remember change to false to rollback!!!!!!!!!!!!!!!!!!!!
        cin = 768
        hidden=int(0.8*cin)
        drop=0.4
        if self.mlp_a:
            self.adaptera= Mlp(cin,hidden,cin,drop=drop)
        if self.mlp_b:
            self.adapterb = Mlp(cin, int(0.8*cin), cin, drop=0.2)
        if self.mlp_c:
            self.adapterc = Mlp(cin, hidden, cin, drop=drop)
    ...
    def forward(
        ...
        if self.mlp_a:
            attention_output+=self.adaptera(attention_output)

        layer_output = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output
        )
        if self.mlp_b:
            layer_output+=self.adapterb(layer_output)
        outputs = (layer_output,) + outputs
        if self.mlp_c:
            outputs+=self.adapterc(outputs)
