...
from maskrcnn_benchmark.engine.predictor_glip_auto import GLIPDemo
...
def inference(
    ...
    all_queries[0]=prompt+all_queries[0]
    ...
    for i, batch in enumerate(_iterator):
        ...
        if visualizer is not None:
            ...
            #
            ###
            result_dir = os.path.join(img_output,'img_{}.jpg'.format(i))
            if result_dir:
                imshow(result, result_dir)
            else:
                os.mkdir(img_output)
                imshow(result, result_dir)
