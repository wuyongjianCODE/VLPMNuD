...
class GLIPDemo(object):
    ...
    def visualize_with_predictions(self, 
        ...
        if self.show_mask_heatmaps:
            return self.create_mask_montage(result, top_predictions)
        result = self.overlay_boxes(result, top_predictions, alpha=alpha, box_pixel=box_pixel, len_pro = len_pro)
        result = self.overlay_entity_names(result, top_predictions, prompt, text_size=text_size, text_pixel=text_pixel, text_offset = text_offset, text_offset_original = text_offset_original)
        if self.cfg.MODEL.MASK_ON:
            result = self.overlay_mask(result, top_predictions)
    ...
    def overlay_boxes(self, image, predictions, alpha=0.5, box_pixel = 3 , len_pro = 15):
    ...
    def overlay_entity_names(self, image, predictions,prompt, names=None, text_size=1.0, text_pixel=2, text_offset = 10, text_offset_original = 4):
