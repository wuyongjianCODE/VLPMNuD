...
class Mlp2(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None,hidden_features2=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = nn.Linear(in_features, hidden_features)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,adap_mlp=1):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        if adap_mlp:
            ...
            if self.adap_mlp>2:
                if version_of_adapterc==1:
                    ...
                    self.adapterc=Mlp2(in_features=dim, hidden_features=int(0.8*dim),hidden_features2=int(0.64*dim), act_layer=act_layer, drop=drop)
        self.H = None
        self.W = None
    ...
    def forward(self, x, mask_matrix):
        ...
        # FFN
        # if self.adap_mlp>0:
        #     # print('adapter 1 success!!!')
        #     x= x+ self.adapter(x)
        x = shortcut + self.drop_path(x)
        residual_path_x=x
        # if self.adap_mlp>1:
        #     # print('adapter 2 success!!!')
        #     residual_path_x= residual_path_x+ self.adapterb(residual_path_x)
        x = residual_path_x + self.drop_path(self.mlp(self.norm2(x)))
        if self.adap_mlp > 2:
            # print('adapter 3 success!!!')
           x = x + self.adapterc(x)
        return x
...
def build_swint_backbone(cfg):
    ...
    """
    import time
    print('building adapter-equipped swint!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
